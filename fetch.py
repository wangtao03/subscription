...
BANNED_WORDS = b64decodes(
    "5rOV6L2uIOi9ruWtkCDova4g57uDIOawlCDlip8g5L2/5YqyIOWKsiDliqrlipsg5Yqg5rK5IOWlsyDmnYMg6L+Q5YqoIG9uZ3RhaXdhbg=="
).split()
...
DEBUG_NO_CATEGORIES = os.path.exists("local_NO_CATEGORIES")
DEBUG_NO_RPOLICY = os.path.exists("local_NO_RPOLICY")
DEBUG_NO_RULES = os.path.exists("local_NO_RULES")
...
session.headers["User-Agent"] = (
    "Mozilla/5.0 (X11; Linux x86_64) Clash-verge/v2.3.1 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36 Edg/114.0.1823.58"
)
...
class Node:
    ...
    def load_url(self, url: str):
        ...
        if self.type == "hy2":
            self.type = "hysteria2"
        # === Fix end ===
        loader: Optional[Callable[[str, str], None]] = getattr(
            self, "_load_" + self.type, None
        )
        if loader:
            loader(url, dt)
        else:
    ...
    @property
    def url(self) -> str:
        handler: Optional[Callable[[Node.DATA_TYPE], str]] = getattr(
            self, "_url_" + self.type, None
        )
        if handler:
...
def main():
    ...
    try:
        ...
        if DEBUG_NO_CATEGORIES:
            # !!! JUST FOR DEBUGING !!!
            print("!!! 警告：您已关闭 按地区分类节点 !!!")
        else:
            print("正在按地区分类节点...")
